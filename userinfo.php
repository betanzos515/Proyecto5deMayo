...
require "admin/config/Conexion.php"; 
    ...
    if (empty($_POST['studentid'])){
        ...
        $dni = mysqli_real_escape_string($conexion,(strip_tags($_POST["studentid"],ENT_QUOTES)));
        $fecha=date('Y-m-d');
        ...
        if ($fil==0) {
		       ...
		       if ($iddni==$dni) {
		            ...
		            if ($query) {
		        	 ...
		        	 echo "<script>jQuery(function(){    swal({type: 'warning', title: 'Advertencia',text: 'Codigo desconocido, intente de nuevo!',showConfirmButton: false,timer: 1200});   });</script>";
        	...
        	if ($sl=="") {
                ...
                if($nuevaHora>$h_entrada){
                    ...
                    if ($query) {
                    	echo "<script>jQuery(function(){    swal({type: 'success', title: 'Exito',text: 'Salida registrada con exito!',showConfirmButton: false,timer: 1200});   });</script>";
                    } else {
